...
<body>
  ...
  if (!empty($_GET['available_sizes'])) {
    $sql .= " AND available_sizes LIKE ?";
    $params[] = '%' . $_GET['available_sizes'] . '%';
    $types .= "s";
  }
  ...
  if (!empty($_GET['sort']) && isset($sortOptions[$_GET['sort']])) {
    $orderBy = " ORDER BY " . $sortOptions[$_GET['sort']];
  }
  $sql .= $orderBy;
  ...
  // Fetch products safely into array
  $products = [];
  while ($row = $result->fetch_assoc()) {
    $row['available_sizes'] = array_map('trim', explode(",", htmlspecialchars($row['available_sizes'], ENT_QUOTES, 'UTF-8')));
    $row['color_options'] = array_map('trim', explode(",", htmlspecialchars($row['color_options'], ENT_QUOTES, 'UTF-8')));
    $row['title'] = htmlspecialchars($row['title'], ENT_QUOTES, 'UTF-8');
    $row['main_color'] = htmlspecialchars($row['main_color'], ENT_QUOTES, 'UTF-8');
    $row['isOutOfStock'] = ((int) $row['out_stock'] === 1);
    $products[] = $row;
  }
  $stmt->close();
  ?>
    ...
    <form method="GET" id="filterForm" style="display:flex; flex-wrap:wrap; gap:20px;">
      ...
      <div class="filter-group">
        ...
        <select name="category" id="category">
          ...
          <option value="Premium collection" <?= ($_GET['category'] ?? '') == "Premium collection" ? 'selected' : '' ?>>
            Premium collection</option>
          <option value="Sweatshirts" <?= ($_GET['category'] ?? '') == "Sweatshirts" ? 'selected' : '' ?>>Sweatshirts
          </option>
  ...
  <div class="product-container">
    <?php if (!empty($products)) {
      ...
      <div class="product-card">
        ...
        <!-- Fullscreen Modal -->
        <div id="fullscreenModal<?= $row['id'] ?>" class="fullscreen-modal">
          <span class="close-modal">&times;</span>
          <img class="fullscreen-content" id="fullscreenImg<?= $row['id'] ?>">
        </div>
        ...
        <?php else: ?>
          <form method="POST" action="add_to_cart.php">
            ...
            <div style="display:flex; gap:10px;">
              <?php foreach ($sizes as $size) { ?>
                <label class="size-btn">
                  <input type="radio" name="product_size" value="<?= $size ?>" required style="display:none;">
                  <span><?= $size ?></span>
                </label>
              <?php } ?>
            </div>
            <div class="actions">
              ...
              <button type="submit" name="add_to_cart" class="btn-cart"><i class="fa-solid fa-cart-shopping"></i> Add to
                Cart</button>
          </form>
