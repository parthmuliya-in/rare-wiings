  ...
  <div class="product-container">
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      <div class="product-card">
        <img id="mainImage<?= $row['id'] ?>" src="uploads/<?= $row['front_image'] ?>" class="main-img">

        <div class="thumbs">
          <img src="uploads/<?= $row['front_image'] ?>" onclick="changeImage<?= $row['id'] ?>(this.src)">
          <img src="uploads/<?= $row['back_image'] ?>" onclick="changeImage<?= $row['id'] ?>(this.src)">
        ...
        <h4>₹<?= $row['price'] ?></h4>
          ...
          <div class="actions">
            <?php if ($isOutOfStock): ?>
          <!-- Out of Stock Label -->
          <span class="out-of-stock" style=" display: inline-block;color: #444;font-weight: bold;padding: 5px 10px;
          border: 2.5px #444 solid;border-radius: 5px;font-size: 20px;text-transform: uppercase;letter-spacing: 0.5px;
          margin-bottom: 47px;"><i class="fa-solid fa-ban"></i> Out of Stock</span>
  <?php endif; ?>

      </div>
      <button class="btn btn-primary view-details" data-id="<?= $row['id']; ?>">View Details</button>
    </div>
